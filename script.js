...
function initChatbot() {
    ...
    const chatbotHTML = `
        <div id="chatbot-container" class="chatbot-hidden">
                ...
                <div class="chatbot-header-info">
                    ...
                    <div>
                        ...
                        <span class="chatbot-header-subtitle">
                            <span class="chatbot-status-dot"></span>APR Assistant
                        </span>
                    </div>
                </div>
                <button id="chatbot-close-btn" title="Close chat"><i class="fas fa-times"></i></button>
            </div>
            ...
            <div class="chatbot-input-container">
                ...
                <button id="chatbot-send-btn" title="Send message"><i class="fas fa-paper-plane"></i></button>
            </div>

            <div class="chatbot-footer">APR - AfterClass</div>
        </div>

        <button id="chatbot-toggle-btn" title="Chat with APR">
    ...
    const STORAGE_KEY = 'afterclass_chat_history';
    ...
    function renderMarkdown(raw) {
        ...
        // wrap consecutive <li> that aren't already wrapped
        s = s.replace(/(<li>[\s\S]*?<\/li>)(?!\s*<\/[uo]l>)/g, (match) => {
        ...
        s = s.replace(/\n/g, '<br>');
        // Clean up <br> right after/before block elements
        s = s.replace(/<br>\s*(<\/?(?:pre|ul|ol|li)>)/g, '$1');
        s = s.replace(/(<\/?(?:pre|ul|ol|li)>)\s*<br>/g, '$1');
    ...
    async function handleSend() {
        ...
        try {
            ...
            if (res.ok && data.reply) {
                ...
                addMessage('⚠️ ' + (data.error || 'Something went wrong. Please try again.'), 'bot-error');
            }
        } catch (err) {
            removeTyping();
            addMessage(
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        setTimeout(() => (toggleBtn.style.display = 'none'), 350);
        inputField.focus();
    });
    ...
    closeBtn.addEventListener('click', () => {
        ...
        requestAnimationFrame(() => (toggleBtn.style.transform = 'scale(1)'));
    });
    ...
    if (chatHistory.length === 0) {
        addMessage(
            "👋 **Hello!** I'm **APR** — your personal engineering tutor.\n\n" +
            "I can help with:\n" +
            "- 📚 **Subject explanations** (DSA, DBMS, OS, ML, Networks…)\n" +
            "- 💻 **Code examples** & debugging\n" +
            "- 📝 **Exam preparation** tips & PYQ patterns\n" +
